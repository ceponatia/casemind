...
- **Types:** `TenantScopedRecord`, `CriminalCase`, `NACase`, `Person`, `DocumentRecord`, `EvidenceRecord`, `FamilyUnit`, `ServicePlan`, `AuditLogEntry`, `NotificationRecord`, `CalendarEvent`, `AiInteractionPlaceholder`, `TenantRecord`, `UserRecord`, and `New*` input variants
...
- **Seeds:** `createDeterministicSeedSet()`
- **Testing:** `InMemoryCaseRepository`, `runCaseRepositoryContractSuite()`

## Dependencies

- **Production:** `@casemind/platform-config` (workspace), `@prisma/client`, `mongodb`, `pg`
...
- **Scripts use `@casemind/platform-config`** to resolve local connection strings. This is the only production cross-package dependency.
